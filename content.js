...
const TRACKS = {
  reading: { name:"Silly Sounds", color:"#ff5c8a", items:[
    ...
    {e:"🔲",t:"Sound boxes",     g:"Listen to the word! Tap a box for each SOUND you hear — say each sound out loud together. c-a-t has three sounds! Boxes are for sounds, not letters.", act:"gen",gen:"soundbox", bench:["ELA.1.F.1.2","ELA.K.F.1.2"]},
    {e:"🧩",t:"Word family",     g:"Pick an ending like -at. Drag or tap a letter in front — c makes CAT, b makes BAT, h makes HAT! Hear each new word. Silly words count too!", act:"gen",gen:"wordfamily", bench:["ELA.K.F.1.3","ELA.1.F.1.3"]},
  ...
  math: { name:"Number Fun", color:"#3aa0ff", items:[
    ...
    {e:"👀",t:"Quick Peek",     g:"Look fast! I'll show you some dots. How many do you see — WITHOUT counting? Just peek and tap the number! Any guess is great.", act:"gen",gen:"subitize", bench:["MA.1.NSO.2.1"]},
    ...
    {e:"🐸",t:"Number line hops",g:"Start on a number, then HOP the frog! Each hop is +1. Count out loud as it jumps to the answer!", act:"gen",gen:"numberline", bench:["MA.1.NSO.2.2","MA.1.AR.1.2"]},
  ...
  science: { name:"Science", color:"#2fbf71", items:[
    ...
    {e:"🧪",t:"How do you know?",g:"Tap a tool for finding out! A magnifier, a telescope, a looking glass — these help us LOOK and TRY things. That's how scientists really know!", act:"tap",tiles:["🔬","🔭","🔎"],answer:null, bench:["SC.1.N.1.4"]},
    {e:"🌱",t:"Living or not?",  g:"Tap each one! A dog and a flower are ALIVE — they grow, eat, and drink water. A rock and a spoon are NOT alive. Tap them all and see!", act:"tap",tiles:["🐶","🪨","🌻","🥄"],answer:null, bench:["SC.1.L.14.3"]},
    {e:"🌻",t:"Plant parts",     g:"Tap each part of the plant! A flower up top, green leaves, a stem, and roots down in the soil. Tap them all!", act:"tap",tiles:["🌸","🍃","🌱","🪴"],answer:null, bench:["SC.1.L.14.2"]},
    {e:"🐣",t:"Baby & grown-up", g:"Tap the baby animals and their mamas! Babies look a LOT like their mamas — but each one is special and a little different!", act:"tap",tiles:["🐱","🐈","🐤","🐔"],answer:null, bench:["SC.1.L.16.1"]},
    ...
    {e:"🧺",t:"Sort it",         g:"Tap each one! An elephant is BIG, a mouse is little. A balloon floats, a rock sinks. Tap them all and notice how they're different!", act:"tap",tiles:["🐘","🐭","🎈","🪨"],answer:null, bench:["SC.1.P.8.1"]},
    ...
    {e:"☀️",t:"Sunny & safe",    g:"Tap each sunny picture! The sun helps flowers grow and warms us. A hat, sunscreen, and cool shades keep us safe. Tap them all!", act:"tap",tiles:["🌻","🧢","🧴","😎"],answer:null, bench:["SC.1.E.5.4"]},
  ]},
  ...
  social: { name:"My World", color:"#ff8c42", items:[
    ...
    {e:"🤝",t:"Kind things",      g:"Tap all the kind things! Sharing, hugging, and helping — every kind thing makes someone feel good. Tap as many as you like!", act:"tap",tiles:["🤝","🫂","🤗"],answer:null, bench:["SS.1.C.2.4"]},
    ...
    {e:"🕯️",t:"Now or long ago",   g:"Tap each picture! Candles and horses are from LONG AGO. Lamps and cars are from NOW. Tap them all and see how things changed!", act:"tap",tiles:["🕯️","💡","🐎","🚗"],answer:null, bench:["SS.1.A.2.2"]},
    ...
    {e:"🎖️",t:"Meet a hero",       g:"Long ago, there were brave and kind people who helped others! Tap each hero and hear a little about them.", act:"tap",tiles:["👮","🚒","🎖️"],answer:null, bench:["SS.1.A.2.4"]},
    {e:"🌅",t:"My day in order",    g:"Tap each part of your day! First we wake up, then we eat lunch, then it's bedtime. Tap them all in a row!", act:"tap",tiles:["🌅","🍽️","🌙"],answer:null, bench:["SS.1.A.3.1"]},
    ...
    {e:"🍎",t:"Need it or want it", g:"Tap each one! Food and water are things we NEED to live. A toy and candy are fun WANTS. Both are okay to tap!", act:"tap",tiles:["🍎","🧸","💧","🍬"],answer:null, bench:["SS.1.E.1.2"]},
    {e:"🍔",t:"A thing or a help",  g:"Tap each one! A hamburger and a car are THINGS you can hold. Sweeping and a doctor's care are HELPS that people do. Tap them all!", act:"tap",tiles:["🍔","🧹","👩‍⚕️","🚗"],answer:null, bench:["SS.1.E.1.3"]},
  ]},
};
